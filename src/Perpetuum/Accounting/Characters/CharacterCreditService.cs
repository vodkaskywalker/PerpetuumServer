...
namespace Perpetuum.Accounting.Characters
{
    public class CharacterCreditService : ICharacterCreditService
    {
        ...
        {
            double credit = Db.Query()
                .CommandText("select credit from characters with (UPDLOCK) where characterid = @characterID")
        ...
        {
            Db.Query()
                .CommandText("update characters with (UPDLOCK) set credit = @credit where characterid = @characterID")
                .SetParameter("@characterID", characterID)
